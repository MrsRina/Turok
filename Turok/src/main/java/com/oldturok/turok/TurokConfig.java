...
public class TurokConfig {
	...
	public static JsonObject create_list_friends(SettingsRegister setting) {
		JsonObject object = new JsonObject();

		for (Map.Entry<String, SettingsRegister> entry : setting.registerHashMap.entrySet()) {
			object.add(entry.getKey(), create_list_friends(entry.getValue()));
		}
	...
	public static void start_save_friends(OutputStream stream) throws IOException {
		Gson gson_gson = new GsonBuilder().setPrettyPrinting().create();
		
		String string_json = gson_gson.toJson(create_list_friends(SettingsRegister.ROOT));
	...
	public static void save_friends(Path path) throws IOException {
		start_save_friends(Files.newOutputStream(path));
	}

	public static void load_friends(Path path) throws IOException {
		InputStream stream = Files.newInputStream(path);

		load_list_friends(stream);

		stream.close();
	}

	public static void load_list_friends(InputStream stream) {
		try {
			start_load_friends(new JsonParser().parse(new InputStreamReader(stream)).getAsJsonObject());
		} catch (IllegalStateException exc) {
			TurokMod.turok_log.error("Failed to load the friends file. Try delet it.");

			start_load_friends(new JsonObject());
		}
	}

	public static void start_load_friends(JsonObject json) {
		load_friends(SettingsRegister.ROOT, json);
	}

	public static void load_friends(SettingsRegister setting, JsonObject json) {
		for (Map.Entry<String, JsonElement> entry : json.entrySet()) {
			String key = entry.getKey();
			...
			if (setting.registerHashMap.containsKey(key)) {
				load_friends(setting.subregister(key), element.getAsJsonObject());
			} else {
				Setting setting_register = setting.getSetting(key);
